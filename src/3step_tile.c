        ...
        for (int iiy = 0; iiy < tile; ++iiy){
          ...
          if ( (iX+iix) > 1 && (iY+iiy) > 1){
            ...
            if ( (iX+iix) > 2 && (iY+iiy) > 2){
              ...
              #pragma ivdep
              for (int iPop = 0; iPop < 9; ++iPop) {
                int nextX = iX+iix-2 + c[iPop][0];
                int nextY = iY+iiy-2 + c[iPop][1];
...
#if 1
// use unrolling and remove row_index %
// support tile_size doesn't need to divide exactly to lx or ly
// support ZGB boundary condition
// For test only LBM without ZGB, simply comment #define ZGB in lb.h
...
#pragma omp parallel default(shared)
{
  ...
  if (my_lx[0] == 1) {
    ...
    for (iY = 1; iY <= ly; ++iY) {
      ...
      if (iY > 1){
        #ifdef ZGB
        if (iY > 2) {
          computeMacros(sim->tmpLattice[lx-1][iY-1].fPop, &rho1, &ux1, &uy1);
          computeMacros(sim->tmpLattice[lx-2][iY-1].fPop, &rho2, &ux2, &uy2);
          pressureBoundary[iY-1].rho = 4./3.*rho1 - 1./3.*rho2;
          pressureBoundary[iY-1].uPar = 0.;
        }
        #endif
  ...
  for (int outerX = my_lx[0] + 2; outerX <= my_lx[1]; outerX += tile) {
    for (int outerY = 1; outerY <= ly ; outerY += tile) {
      ...
      for (int innerX = outerX; innerX <= innerX_max; ++innerX) {
        ...
        #ifdef ZGB
        if (innerX == my_lx[1] || innerX == (my_lx[1] - 1) || innerX == (lx - 2) || innerX == (lx - 3)) {
        #else
  ...
  for (iX = 1; iX <= lx; ++iX){
    ...
    // compute 3rd c&s on row iX=lx
    if ( iX == lx ){
      computeMacros(sim->lattice[lx-1][iY-1].fPop, &rho1, &ux1, &uy1);
      computeMacros(sim->lattice[lx-2][iY-1].fPop, &rho2, &ux2, &uy2);
      pressureBoundary[iY-1].rho = 4./3.*rho1 - 1./3.*rho2;
      pressureBoundary[iY-1].uPar = 0.;
    }
    #endif
...
// use unrolling and remove row_index %
// support tile_size doesn't need to divide exactly to lx or ly
// The following for testing only the LBM equation, and don't support ZGB boundary condition
void step3CollideStreamTileOMP(Simulation* sim) {
  int lx = sim->lx, ly = sim->ly;
